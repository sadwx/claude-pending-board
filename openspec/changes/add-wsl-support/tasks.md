...
- [ ] In `crates/adapters/src/wezterm.rs::spawn_resume`, branch on whether the entry's `cwd` came with a `wsl_distro` (the adapter signature must be extended to receive the distro — see note below).
...
- [ ] Threading note: `Adapter::spawn_resume` currently takes `(cwd, session_id)`. Either widen to `(cwd, session_id, wsl_distro: Option<&str>)` or pass the whole `Entry`. The wider tuple is cheaper to land; passing `Entry` is more future-proof. **Pick widen-tuple for PR-C** to keep the diff small.
...
- [ ] Restore the `claude-pending-board-WSL` mention in the README's Status / Requirements section.
