...
- **Manifest sanitization runs in three places.** `crates/app/src/plugin_install.rs::sanitize_installed_plugin_json()` strips foreign-platform hook entries from the cached `plugin.json` so `/hooks` only shows entries that can run on the current OS. Triggered after `install()` (one-click setup card), as a best-effort startup task in `main.rs setup()`, and via the `--sanitize-manifest` CLI flag for users who don't keep the tray app running. All three paths must keep working — if you add a new install path, wire sanitize into it too.
